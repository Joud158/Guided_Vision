from typing import Any, Dict, List

import cv2  # noqa: F401  # kept for potential future use (debug, drawing)
from ultralytics import YOLO

from distance_direction import compute_distance_and_direction


class DangerDetector:
    """Wrapper around a YOLO model to detect dangerous objects."""

    def __init__(self, model_path: str) -> None:
        self.model_path = model_path
        self.model = YOLO(model_path)
        # Mapping from class id to human-readable class name
        self.class_names = self.model.names

    def detect_dangers(self, frame) -> List[Dict[str, Any]]:
        """Run YOLO on a frame and return a list of detected dangers.

        Each item has:
        - label: class name (e.g. 'car')
        - confidence: float
        - distance_m: rough distance in meters
        - direction: LEFT / RIGHT / FRONT
        """
        results = self.model(frame, verbose=False)[0]
        h, w = frame.shape[:2]
        dangers: List[Dict[str, Any]] = []

        for box in results.boxes:
            cls_id = int(box.cls.item())
            label = self.class_names.get(cls_id, f"class_{cls_id}")
            conf = float(box.conf.item())
            x1, y1, x2, y2 = box.xyxy[0].tolist()

            distance_m, direction = compute_distance_and_direction(
                x1=x1,
                y1=y1,
                x2=x2,
                y2=y2,
                frame_width=w,
                frame_height=h,
                label=label,
            )

            dangers.append(
                {
                    "label": label,
                    "confidence": conf,
                    "distance_m": distance_m,
                    "direction": direction,
                }
            )

        # Sort by distance (closest first)
        dangers.sort(key=lambda d: d["distance_m"])
        return dangers
